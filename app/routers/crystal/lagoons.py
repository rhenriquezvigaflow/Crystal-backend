...
def crystal_get_lagoon_layout_config(
    lagoon_id: str,
    db: Session = Depends(get_db),
    user: dict = Depends(get_current_user),
):
    ...
    logger.info(
        "[API] endpoint_response_summary endpoint=/api/crystal/lagoons/%s/layout-config method=GET user_id=%s layout_id=%s mapped_elements=%s",
        lagoon_id,
        user_id,
...
def crystal_update_lagoon_layout_config(
    lagoon_id: str,
    ...
    logger.info(
        "[API] endpoint_response_summary endpoint=/api/crystal/lagoons/%s/layout-config method=PUT user_id=%s layout_id=%s upsert_count=%s",
        lagoon_id,
        user_id,
        config.mapping.layout_id,
