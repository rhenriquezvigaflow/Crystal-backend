...
-- Remove stale 1-hour lagoon definitions when the same lagoon already has
-- the canonical 6-hour definition.
WITH duplicate_definitions AS (
    SELECT d1.id
    FROM public.alarm_definition d1
    WHERE d1.alarm_type = 'comm_loss'
      AND d1.tag_id IS NULL
      AND d1.code LIKE '%_1h'
      AND EXISTS (
          SELECT 1
          FROM public.alarm_definition d2
          WHERE d2.lagoon_id = d1.lagoon_id
            AND d2.alarm_type = 'comm_loss'
            AND d2.tag_id IS NULL
            AND d2.code LIKE '%_6h'
      )
)
DELETE FROM public.alarm_notification_rule
WHERE alarm_definition_id IN (SELECT id FROM duplicate_definitions);
...
DELETE FROM public.alarm_event
WHERE alarm_definition_id IN (SELECT id FROM duplicate_definitions);
...
DELETE FROM public.alarm_definition
WHERE id IN (SELECT id FROM duplicate_definitions);

UPDATE public.alarm_definition
SET
    code = regexp_replace(code, '_1h$', '_6h'),
    name = regexp_replace(name, '1 hora$', '6 horas'),
    description = CASE
        WHEN description IS NULL THEN NULL
        ELSE regexp_replace(description, '1 hora', '6 horas', 'g')
    END,
    condition = jsonb_set(
